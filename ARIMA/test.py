...
def fetch_data():
    ...
    query = "SELECT ntime, label, paintwear, price FROM ak_47_野荷"
...
for label, prices in result.items():
    model = ARIMA(prices, order=(9, 3, 0))  # (p,d,q)参数需要调整
    model_fit = model.fit()
...
for label in result.keys():
    ...
    plt.plot(dates, result[label], label=f'AK-47|野荷{label} 真实值')
    plt.plot(future_dates, predictions[label], label=f'AK-47|野荷{label} 预测值', color='r')

    # 添加价格走势线
    first_real_value = result[label].iloc[0]
    last_forecast_value = predictions[label].iloc[-1]
    plt.plot([dates[0], future_dates[-1]], [first_real_value, last_forecast_value], label='价格走势', color='g',
             linestyle='--')

    plt.legend()
    plt.xlabel('Date')
    plt.ylabel('Price')
    plt.title(f'ARIMA Forecast of AK-47|野荷 Skin Prices for {label}')
    plt.show()

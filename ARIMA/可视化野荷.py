...
def fetch_data():
    ...
    data_query = "SELECT date, forecast_price FROM data_ak_47_野荷 ORDER BY date DESC LIMIT 7"
    data_df = pd.read_sql(data_query, connection)

    # 读取 ak_47_可燃冰 表的 ntime 和 price 字段
    real_query = "SELECT ntime, price FROM ak_47_野荷"
...
def plot_prices(data_df, real_df):
    ...
    plt.xlabel('日期')
    plt.ylabel('价格')
    plt.title('ARIMA Forecast of AK-47|野荷 Skin Prices')
    plt.legend()
    plt.grid(True)
    plt.show()

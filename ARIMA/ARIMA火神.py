...
def fetch_data():
    ...
    query = "SELECT ntime, label, paintwear, price FROM ak_47_火神"
...
for label, prices in result.items():
    model = ARIMA(prices, order=(10, 2, 0))  # (p,d,q)参数需要调整
    model_fit = model.fit()
...
for label in result.keys():
    ...
    plt.plot(dates, result[label], label=f'AK-47|火神{label} 真实值')
    plt.plot(future_dates, predictions[label], label=f'AK-47|火神{label} 预测值', color='r')
    plt.legend()
    plt.xlabel('Date')
    plt.ylabel('Price')
    plt.title(f'ARIMA Forecast of AK-47|火神 Skin Prices for {label}')
    plt.show()
